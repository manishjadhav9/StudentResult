...
class AdobeFontMetrics {
  ...
  function write($file, $encoding = null) {
    ...
    $this->startDepartment("FontMetrics", 4.1);
    ...
    if ($glyphIndexArray) {
      ...
      $this->startDepartment("CharMetrics", count($hmtx));
      ...
      $this->endDepartment("CharMetrics");

      $kern = $font->getData("kern", "subtable");
      $tree = $kern["tree"];

      if (!$encoding && is_array($tree)) {
        $this->startDepartment("KernData");
        $this->startDepartment("KernPairs", count($tree, COUNT_RECURSIVE) - count($tree));
        ...
        $this->endDepartment("KernPairs");
        $this->endDepartment("KernData");
      }
    }

    $this->endDepartment("FontMetrics");
  }
  ...
  function startDepartment($name, $value = "") {
    $this->addLine("Start$name $value");
  }

  function endDepartment($name) {
