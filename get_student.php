...
if (!empty($_POST["classid"])) {
  ...
  if (!is_numeric($cid)) {
    ...
    $stmt = $dbh->prepare("SELECT StudentName,StudentId FROM tblstudents WHERE YearId= :id order by StudentName");
...
if (!empty($_POST["classid1"])) {
  ...
  if (!is_numeric($cid1)) {
    ...
    $status = 0;
    $stmt = $dbh->prepare("SELECT tblsubjects.SubjectName,tblsubjects.id FROM tblsubjectcombination join  tblsubjects on  tblsubjects.id=tblsubjectcombination.SubjectId WHERE tblsubjectcombination.YearId=:cid and tblsubjectcombination.status!=:stts order by tblsubjects.SubjectName");
...
if (!empty($_POST["studclass"])) {
  ...
  $id = $dta[0];
  $id1 = $dta[1];
  $query = $dbh->prepare("SELECT StudentId,YearId FROM tblresult WHERE StudentId=:id1 and YearId=:id ");
