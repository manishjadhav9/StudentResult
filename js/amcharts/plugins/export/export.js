...
( function() {
	AmCharts[ "export" ] = function( chart, config ) {
		...
		var _this = {
			...
			/**
			 * Recursive method which crawls upwards to gather the requested yearname
			 */
			gatherYearName: function( elm, className, limit, lvl ) {
				var value, lvl = lvl ? lvl : 0,
					limit = limit ? limit : 3;
				...
				if ( _this.isElement( elm ) ) {
					...
					if ( !value && lvl < limit ) {
						return _this.gatherYearName( elm.parentNode, className, limit, lvl + 1 );
					} else if ( value ) {
						value = elm;
					}
				}
			...
			capture: function( options, callback ) {
				...
				for ( i1 = 0; i1 < svgs.length; i1++ ) {
					...
					group.has.legend = _this.gatherYearName( group.parent, _this.setup.chart.classNamePrefix + "-legend-div", 1 );
					group.has.panel = _this.gatherYearName( group.parent, _this.setup.chart.classNamePrefix + "-stock-panel-div" );
					group.has.scrollbar = _this.gatherYearName( group.parent, _this.setup.chart.classNamePrefix + "-scrollbar-chart-div" );
					...
					fabric.parseSVGDocument( group.svg, ( function( group ) {
						return function( objects, options ) {
							...
							if ( group.svg.parentNode && group.svg.parentNode.getElementsByTagName ) {
								var balloons = group.svg.parentNode.getElementsByYearName( _this.setup.chart.classNamePrefix + "-balloon-div" );
			...
			construct: function() {
				...
				if ( _this.isSupported() ) {
					...
					// ADD CLASSNAMES
					_this.setup.chart.addYearNames = true;
					// REFERENCE
